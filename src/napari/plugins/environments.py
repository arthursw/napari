"""Public execution API for isolated plugin worker commands.

Napari reconciles every declared worker environment during application
startup. Plugin code can execute and cancel commands during the session, but
cannot install, update, remove, or otherwise manage environments at runtime.
"""
...
class WorkerContext(Protocol):
    """Progress and cancellation interface supplied inside a worker process."""
    ...
    def update(
        ...
        *,
        ...
        """Report progress to the calling plugin task."""
        ...


class PluginTaskState(Enum):
    """Lifecycle state of a worker command."""
...
class PluginTaskPhase(Enum):
    """Current runtime phase of a worker command."""
...
class PluginTaskProgress:
    """Most recent progress reported by a worker command."""
...
class PluginEnvironmentError(RuntimeError):
    """Base exception for managed plugin worker failures."""
...
class PluginEnvironmentUnavailableError(PluginEnvironmentError):
    """The command's startup-managed environment is unavailable."""
...
class PluginTaskCanceledError(PluginEnvironmentError):
    """A managed plugin worker command was canceled."""
...
def _remove_task_observer(observer: Callable[[PluginTask[Any]], Any]) -> None:
    with _task_hook_lock, suppress(ValueError):
...
class PluginTask(Generic[T]):
    """An observable and cancellable worker command."""
    ...
    def add_done_callback(
        self, callback: Callable[[PluginTask[T]], Any]
    ) -> None:
        with self._lock:
            dispatch = self._done.is_set()
            if not dispatch:
    ...
    def cancel(self, reason: str | None = None) -> bool:
        with self._lock:
            if self._state.terminal or self._cancellation_requested:
                return False
        ...
        try:
            ...
            logger.exception('Plugin task cancellation callback failed')
            accepted = False
    ...
    def _set_running(self, phase: PluginTaskPhase, message: str) -> None:
        with self._lock:
    ...
    def _set_canceled(self) -> None:
        error = PluginTaskCanceledError(
            self.cancellation_reason or 'Plugin task was canceled',
            phase=self.phase,
        )
        with self._lock:
            if self._state.terminal:
...
def execute_worker_command(
    ...
    """Execute a qualified worker command declared by a plugin manifest.

    Napari installs or updates every declared environment during startup.
    This call never provisions an environment. If startup setup did not make
    the command ready, the returned task fails immediately with guidance to
    restart napari and retry setup.
    """
