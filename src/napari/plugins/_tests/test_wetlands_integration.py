...
def _require_supported_wetlands() -> None:
    ...
    try:
        supported = Version('2.3') <= Version(version) < Version('2.4')
    except InvalidVersion:
        supported = False
    if not supported:
        pytest.skip(f'Wetlands >=2.3,<2.4 is required, found {version}')
...
def _recipe(
    ...
    *,
    ...
    return EnvironmentRecipe(
        ...
        conda=(f'numpy=={np.__version__}', 'pip'),
        pypi=(f'typing-extensions=={dependency_version}',),
        channels=('conda-forge',),
        worker_package=package,
...
def test_real_wetlands_isolates_reuses_and_cancels_workers(
    ...
    environments = tuple(
        _SnapshotEnvironment(
            plugin=recipe.plugin,
            plugin_display_name=recipe.plugin,
            plugin_enabled=True,
            environment_id=recipe.environment_id,
            display_name='Worker',
            recipe=recipe,
    ...
    monkeypatch.setattr(
        manager_module,
        '_build_snapshot',
        lambda backend: _StartupSnapshot(
            environments=tuple(
                _SnapshotEnvironment(
                    **{
                        **environment.__dict__,
                        'recipe': environment.recipe.__class__(
                            **{
                                **environment.recipe.__dict__,
                                'worker_content_identity': (
                                    backend.worker_content_identity(package)
                                ),
                            }
                        ),
                    }
    ...
    try:
        manager.start_reconciliation().result(900)
        old_progress = []
